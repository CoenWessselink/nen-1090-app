...
try {
  Write-Section "Phase 11 v7 full proof run"
  ...
  Invoke-StepWithTimeout -Name "playwright-install" -Command "npx playwright install" -TimeoutMinutes $InstallTimeoutMinutes -WorkingDirectory $RepoPath -LogDirectory $logDirectory
  ...
  Invoke-StepWithTimeout -Name "smoke-suite" -Command "npx playwright test tests/e2e/smoke --project=desktop-chromium --reporter=line" -TimeoutMinutes $SmokeTimeoutMinutes -WorkingDirectory $RepoPath -LogDirectory $logDirectory -EnvironmentVariables $envs
