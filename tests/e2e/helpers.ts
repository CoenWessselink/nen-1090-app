...
export async function seedSession(
  page: Page,
  options: SessionSeedOptions | string = {},
) {
  const normalized: SessionSeedOptions =
...
export async function stubAuthApi(
  page: Page,
  options: SessionSeedOptions | string = {},
) {
  const normalized: SessionSeedOptions =
...
export async function stubCommonApi(page: Page) {
  await page.route("**/api/v1/notifications**", async (route) =>
    fulfillJson(route, { items: [], unread: 0 }),
  );
...
export async function ensureAuthenticated(
  ...
  const tenantInput = page
    .locator(
      'input[name="tenant"], input[placeholder*="tenant" i], input[aria-label*="tenant" i]',
    )
    .first();
  ...
  const loginButton = page
    .getByRole("button", { name: /inloggen|login|aanmelden/i })
    .first();
...
export async function fillProjectForm(
  ...
  const omschrijving = await visibleFirst([
    ...
    page.locator(
      'input[name="name"], textarea[name="description"], input[name="description"]',
    ),
  ]);
  ...
  for (const locator of excLocators) {
    if (await locator.count()) {
      await locator
        .first()
        .selectOption(values.executieklasse ?? "EXC3")
        .catch(async () => {
          await locator.first().click();
          await page
            .getByRole("option", { name: values.executieklasse ?? "EXC3" })
            .click()
            .catch(() => {});
        });
      break;
  ...
  for (const locator of statusLocators) {
    if (await locator.count()) {
      await locator
        .first()
        .selectOption(values.status ?? "in-uitvoering")
        .catch(async () => {
          await locator.first().click();
          await page
            .getByRole("option", {
              name: new RegExp(values.status ?? "in-uitvoering", "i"),
            })
            .click()
            .catch(() => {});
        });
...
export async function confirmDelete(page: Page) {
  const confirmButton = await visibleFirst([
    page.getByRole("button", {
      name: /project verwijderen|verwijderen|bevestig|ja,? verwijder/i,
    }),
...
export async function bootstrapAuthenticatedPage(
  ...
  await page.route("**/api/v1/projects**", async (route) => {
    return fulfillJson(route, {
      page: 1,
      limit: 25,
      total: seededProjects.length,
      items: seededProjects,
    });
  });

  await page.route("**/api/v1/projects/*/assemblies**", async (route) => {
    return fulfillJson(route, {
      page: 1,
      limit: 25,
      total: 1,
      items: [
        { id: "assembly-001", name: "Assembly A", status: "in-uitvoering" },
      ],
    });
  });

  await page.route("**/api/v1/projects/*/welds**", async (route) => {
    return fulfillJson(route, {
      page: 1,
      limit: 25,
      total: 1,
      items: [
        { id: DEFAULT_WELD_ID, weld_number: "L-001", status: "in-controle" },
      ],
    });
  });

  await page.route("**/api/v1/projects/*/compliance**", async (route) => {
    return fulfillJson(route, {
      score: 82,
  ...
  await page.route("**/api/v1/projects/*/documents**", async (route) => {
    return fulfillJson(route, {
      ...
      limit: 25,
      total: 1,
      items: [{ id: "doc-001", filename: "wps-demo.pdf", category: "WPS" }],
    });
  });

  await page.route("**/api/v1/reports**", async (route) => {
    return fulfillJson(route, {
      items: [{ id: "report-001", title: "Weekrapport", status: "gereed" }],
    });
  });
...
export async function openFirstProject360(page: Page) {
  ...
  const firstRow = page
    .locator("tr")
    .filter({ hasText: /P-20\d{2}|Demo project/i })
    .first();
  await expect(firstRow).toBeVisible();
  const openButton = firstRow
    .getByRole("button", { name: /open project 360|open/i })
    .first();
  await openButton.click();
  await expect(page).toHaveURL(/\/projecten\/.+\/(overzicht)?/i);
  await expect(
    page.getByRole("button", { name: /nieuwe assembly/i }),
  ).toBeVisible();
}
