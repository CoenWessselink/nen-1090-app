...
export function useUpsertWeldInspection(projectId: string | number, weldId: string | number) {
  ...
  return useMutation({ mutationFn: async (payload: Record<string, unknown>) => await upsertInspectionForWeld(projectId, weldId, payload as never), onSuccess: async () => { await qc.invalidateQueries({ queryKey: ['inspections'] }); await qc.invalidateQueries({ queryKey: ['weld-inspection', projectId, weldId] }); await qc.invalidateQueries({ queryKey: ['project-welds', projectId] }); await qc.invalidateQueries({ queryKey: ['projects'] }); await qc.invalidateQueries({ queryKey: ['project', projectId] }); await qc.invalidateQueries({ queryKey: ['compliance-overview', projectId] }); await qc.invalidateQueries({ queryKey: ['compliance-missing', projectId] }); await qc.invalidateQueries({ queryKey: ['compliance-checklist', projectId] }); await qc.invalidateQueries({ queryKey: ['ce-dossier', projectId] }); } });
}
