...
export function useCreateInspection() { const qc = useQueryClient(); return useMutation({ mutationFn: async ({ projectId, weldId, payload }: { projectId?: string | number; weldId: string | number; payload: Record<string, unknown> }) => await createInspection(String(projectId || ''), weldId, payload), onSuccess: async (_d, v) => { await qc.invalidateQueries({ queryKey: ['inspections'] }); await qc.invalidateQueries({ queryKey: ['weld-inspection', String(v.projectId || ''), v.weldId] }); await qc.invalidateQueries({ queryKey: ['project-welds', String(v.projectId || '')] }); await qc.invalidateQueries({ queryKey: ['ce-dossier', String(v.projectId || '')] }); await qc.invalidateQueries({ queryKey: ['project', String(v.projectId || '')] }); } }); }
export function useUpdateInspection() { const qc = useQueryClient(); return useMutation({ mutationFn: async ({ inspectionId, payload }: { inspectionId: string | number; payload: Record<string, unknown> }) => await updateInspection(inspectionId, payload), onSuccess: async (_d, v) => { const projectId = String((v.payload as any)?.project_id || (v.payload as any)?.projectId || ''); const weldId = String((v.payload as any)?.weld_id || (v.payload as any)?.weldId || ''); await qc.invalidateQueries({ queryKey: ['inspections'] }); await qc.invalidateQueries({ queryKey: ['inspection-results', v.inspectionId] }); await qc.invalidateQueries({ queryKey: ['inspection-audit', v.inspectionId] }); if (projectId) { await qc.invalidateQueries({ queryKey: ['project', projectId] }); await qc.invalidateQueries({ queryKey: ['project-welds', projectId] }); await qc.invalidateQueries({ queryKey: ['ce-dossier', projectId] }); } if (projectId && weldId) { await qc.invalidateQueries({ queryKey: ['weld-inspection', projectId, weldId] }); } } }); }
