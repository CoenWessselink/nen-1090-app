...
function mapProjectPayload(payload: Pick<ProjectFormValues, 'projectnummer' | 'name' | 'client_name' | 'execution_class' | 'status' | 'start_date' | 'end_date'>) {
  return {
...
export async function getProjectInspections(projectId: string | number, _params?: ListParams) {
  ...
  for (const weld of welds.items || []) {
    const rows = await listRequest<Inspection[] | { items?: Inspection[] }>(`/projects/${projectId}/welds/${weld.id}/inspections`, {} as ListParams);
...
export async function getProjectSelectedMaterials(projectId: string | number) { return (await optionalRequest<Record<string, unknown>[]>([`/projects/${projectId}/selected/materials`])) || []; }
export async function getProjectSelectedWps(projectId: string | number) { return (await optionalRequest<Record<string, unknown>[]>([`/projects/${projectId}/selected/wps`])) || []; }
export async function getProjectSelectedWelders(projectId: string | number) { return (await optionalRequest<Record<string, unknown>[]>([`/projects/${projectId}/selected/welders`])) || []; }
export async function approveAllProject(projectId: string | number) {
  ...
  let approvedWelds = 0;
  if (weldIds.length) {
    const bulkResult = await optionalRequest<Record<string, unknown>>(
...
export async function applyProjectInspectionTemplate(projectId: string | number, templateId?: string | null) {
  ...
  return (await optionalRequest<Record<string, unknown>>([`/projects/${projectId}/apply-inspection-template`], { method: 'POST', body: JSON.stringify({ template_id: templateId, mode: 'merge' }) })) || { ok: true, skipped: true, projectId, templateId };
}
export async function addProjectMaterials(projectId: string | number) { return (await optionalRequest<Record<string, unknown>>([`/projects/${projectId}/materials/add-all`], { method: 'POST' })) || { ok: true, projectId }; }
export async function addProjectWps(projectId: string | number) { return (await optionalRequest<Record<string, unknown>>([`/projects/${projectId}/wps/add-all`], { method: 'POST' })) || { ok: true, projectId }; }
export async function addProjectWelders(projectId: string | number) { return (await optionalRequest<Record<string, unknown>>([`/projects/${projectId}/welders/add-all`], { method: 'POST' })) || { ok: true, projectId }; }
...
export async function updateProjectRecord(id: string | number, payload: Partial<ProjectFormValues>) {
  const response = await optionalRequest<Record<string, unknown>>(
    [`/projects/${id}`],
    { method: 'PATCH', body: JSON.stringify(mapProjectPatch(payload)) },
  ) || await optionalRequest<Record<string, unknown>>(
    [`/projects/${id}`],
    { method: 'PUT', body: JSON.stringify(mapProjectPatch(payload)) },
  );
...
export async function updateProject(id: string | number, payload: ProjectFormValues) {
  const response = await optionalRequest<Record<string, unknown>>(
    [`/projects/${id}`],
    { method: 'PUT', body: JSON.stringify(mapProjectPayload(payload)) },
  ) || await optionalRequest<Record<string, unknown>>(
    [`/projects/${id}`],
    { method: 'PATCH', body: JSON.stringify(mapProjectPayload(payload)) },
  );
  if (response) return normalizeProjectRecord(response);
  return await getProject(id);
}
export async function deleteProject(id: string | number) { await apiRequest(`/projects/${id}`, { method: 'DELETE' }); return { ok: true, id }; }
...
export async function addProjectMaterialLink(projectId: string | number, materialId: string | number) { return (await optionalRequest<Record<string, unknown>>([`/projects/${projectId}/materials`], { method: 'POST', body: JSON.stringify({ ref_id: materialId }) })) || { ok: true, projectId, materialId }; }
export async function removeProjectMaterialLink(projectId: string | number, materialId: string | number) { await apiRequest(`/projects/${projectId}/materials/${materialId}`, { method: 'DELETE' }); return { ok: true, projectId, materialId }; }
export async function addProjectWpsLink(projectId: string | number, wpsId: string | number) { return (await optionalRequest<Record<string, unknown>>([`/projects/${projectId}/wps`], { method: 'POST', body: JSON.stringify({ ref_id: wpsId }) })) || { ok: true, projectId, wpsId }; }
export async function removeProjectWpsLink(projectId: string | number, wpsId: string | number) { await apiRequest(`/projects/${projectId}/wps/${wpsId}`, { method: 'DELETE' }); return { ok: true, projectId, wpsId }; }
export async function addProjectWelderLink(projectId: string | number, welderId: string | number) { return (await optionalRequest<Record<string, unknown>>([`/projects/${projectId}/welders`], { method: 'POST', body: JSON.stringify({ ref_id: welderId }) })) || { ok: true, projectId, welderId }; }
export async function removeProjectWelderLink(projectId: string | number, welderId: string | number) { await apiRequest(`/projects/${projectId}/welders/${welderId}`, { method: 'DELETE' }); return { ok: true, projectId, welderId }; }
