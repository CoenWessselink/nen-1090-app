...
function normalizeMutationResponse(payload: unknown): MasterDataItem {
  ...
  if (data && typeof data === 'object' && !Array.isArray(data)) return data as MasterDataItem;
  const item = record.item;
  if (item && typeof item === 'object' && !Array.isArray(item)) return item as MasterDataItem;
  return record;
}

async function mutationRequest(path: string, method: 'POST' | 'PATCH' | 'PUT', payload?: Record<string, unknown>) {
  const response = await apiRequest<unknown>(path, { method, body: payload === undefined ? undefined : JSON.stringify(payload) });
...
export function createMaterial(payload: Record<string, unknown>) { return mutationRequest('/settings/materials', 'POST', payload); }
export function updateMaterial(materialId: string | number, payload: Record<string, unknown>) { return mutationRequest(`/settings/materials/${materialId}`, 'PATCH', payload); }
...
export function createWelder(payload: Record<string, unknown>) { return mutationRequest('/settings/welders', 'POST', payload); }
export function updateWelder(welderId: string | number, payload: Record<string, unknown>) { return mutationRequest(`/settings/welders/${welderId}`, 'PATCH', payload); }
...
export function createInspectionTemplate(payload: Record<string, unknown>) { return mutationRequest('/settings/inspection-templates', 'POST', payload); }
export async function updateInspectionTemplate(templateId: string | number, payload: Record<string, unknown>) {
  return normalizeMutationResponse(await optionalRequest<MasterDataItem>([`/settings/inspection-templates/${templateId}`], { method: 'PATCH', body: JSON.stringify(payload) }) || await apiRequest<MasterDataItem>(`/settings/inspection-templates/${templateId}`, { method: 'PUT', body: JSON.stringify(payload) }));
}
...
export async function duplicateInspectionTemplate(templateId: string | number) {
  ...
  if (direct) return normalizeMutationResponse(direct);
...
export function createWeldCoordinator(payload: Record<string, unknown>) { return mutationRequest('/settings/weld-coordinators', 'POST', payload); }
export function updateWeldCoordinator(coordinatorId: string | number, payload: Record<string, unknown>) { return mutationRequest(`/settings/weld-coordinators/${coordinatorId}`, 'PATCH', payload); }
...
export function updateCompanySettings(payload: Record<string, unknown>) { return mutationRequest('/settings/company', 'PUT', payload); }
