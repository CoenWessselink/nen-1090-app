...
async function parseError(response: Response): Promise<ApiError> {
  ...
  return new ApiError({
    ...
    details,
    code: typeof details === 'object' && details && 'code' in details ? String((details as { code?: unknown }).code) : undefined,
  });
}
...
function isTenantSessionMismatch(error: ApiError): boolean {
  return error.status === 403 && /tenant header komt niet overeen met de sessie|tenant membership not found/i.test(error.message || '');
}
  ...
  refreshInFlight = (async () => {
    ...
    try {
      ...
      if (!payload.access_token || !user) {
        clearSession();
        useApiStatusStore.getState().markSessionExpired('Je sessie kon niet worden vernieuwd via de bestaande authflow.');
        return false;
      }
...
export function resolveProjectScopedPath(projectId: string | number | null | undefined, scopedPath: string, fallbackPath: string): string {
...
export async function apiRequest<T>(path: string, init?: RequestInit, retryCount = 0): Promise<T> {
  const response = await fetch(`${env.apiBaseUrl}${path}`, { ...init, credentials: 'include', headers: buildHeaders(init) });
  ...
  if (response.status === 401 && retryCount === 0 && !isAuthRefreshCall && !isAuthTerminalCall) {
    ...
    if (refreshed) return apiRequest<T>(path, init, 1);
  }
...
export async function downloadRequest(path: string): Promise<Blob> {
  const response = await fetch(`${env.apiBaseUrl}${path}`, { credentials: 'include', headers: buildHeaders() });
