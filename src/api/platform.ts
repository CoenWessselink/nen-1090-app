...
export function getTenantProfile(tenantId: string | number) {
  return apiRequest<TenantProfile>(`/platform/tenants/${tenantId}/profile`);
}

export function patchTenantProfile(tenantId: string | number, payload: Partial<TenantProfile>) {
  return client.patch<TenantProfile>(`/platform/tenants/${tenantId}/profile`, payload);
}
