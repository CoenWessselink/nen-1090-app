...
function uploadUrl(path: string): string {
  ...
  // Production default: use the same-origin Cloudflare Pages Function proxy.
  // This preserves HttpOnly cookie sessions and lets the proxy inject Authorization.
  return apiPath;
}
...
function headersForUpload(): Headers {
  ...
  // Keep this minimal. Never set Content-Type for FormData; the browser must add the multipart boundary.
...
export function filesFromInput(input: FormData | File | File[]): File[] {
  if (input instanceof File) return input.size > 0 ? [input] : [];
  if (Array.isArray(input)) return dedupeFiles(input.filter((item): item is File => item instanceof File && item.size > 0));
  const files: File[] = [];
...
export async function uploadOne<T = unknown>(path: string, file: File, extra?: Record<string, string | number | boolean | null | undefined>): Promise<T> {
  if (!(file instanceof File) || file.size <= 0) throw new ApiError('Geen bestand geselecteerd voor upload.', 400);
...
export async function uploadMany<T = unknown>(path: string, input: FormData | File | File[], extra?: Record<string, string | number | boolean | null | undefined>): Promise<T[]> {
  ...
  if (!files.length) throw new ApiError('Geen bestand geselecteerd voor upload.', 400);
  const uploaded: T[] = [];
  for (const file of files) uploaded.push(await uploadOne<T>(path, file, extra));
  return uploaded;
}
