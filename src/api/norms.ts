...
function asArray<T>(payload: unknown): T[] {
  ...
  const record = payload as { items?: T[]; data?: T[]; results?: T[]; checklist?: T[] } | null;
  return record?.items || record?.data || record?.results || record?.checklist || [];
}
...
export async function saveWeldInspection(projectIdOrWeldId: string, weldIdOrPayload: string | Record<string, unknown>, maybePayload?: Record<string, unknown>) {
  ...
  const saved = await apiRequest<unknown>(path, { method: 'POST', body: JSON.stringify(body) });
