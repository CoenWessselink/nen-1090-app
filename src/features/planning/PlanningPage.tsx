...
export function PlanningPage() {
  ...
  return (
    <MobilePageScaffold
      title="Planning"
      subtitle="Server-side zoeken, sorteren en status"
      backTo="/projecten"
      rightSlot={
        <button type="button" className="mobile-icon-button" onClick={() => query.refetch()} disabled={query.isFetching} aria-label="Vernieuwen">
          <RefreshCcw size={18} />
        </button>
      }
    >
      <div className="mobile-unified-body">
        <Card>
          <DataTableToolbar
            ...
            center={
              <Badge tone="neutral">
                <CalendarRange size={14} /> Planning · {query.data?.total || rows.length} totaal
              </Badge>
            }
            right={
          ...
          {query.isError ? (
            <ErrorState
              title="Planning niet geladen"
              description="De frontend gebruikt geen fallback-rijen meer. Controleer of het bestaande planning-endpoint beschikbaar is en het verwachte contract teruggeeft."
            />
          ) : null}
