...
import { FileText, Plus, Printer, RefreshCcw, Search, Trash2 } from 'lucide-react';
...
export function InvoiceManagerPage() {
  ...
  return (
    <MobilePageScaffold title="Facturatie" subtitle="Facturen aanmaken, bekijken en downloaden als PDF" rightSlot={<button type="button" className="mobile-icon-button" onClick={reload}><RefreshCcw size={18} /></button>}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, paddingBottom: 32 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          {tenants.length > 1 && (
            <select style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid #cbd5e1', fontSize: 13 }} onChange={(e) => { if (e.target.value) handleCreateForTenant(e.target.value); e.target.value = ''; }}>
        ...
        {/* Search */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', border: '1px solid #e2e8f0', borderRadius: 10, background: '#fff' }}>
          <Search size={14} />
        ...
        {/* Invoice list */}
        {filtered.length === 0 && <div style={{ textAlign: 'center', padding: 32, color: '#94a3b8' }}>Nog geen facturen. Klik "Nieuwe factuur" om er een aan te maken.</div>}
        {filtered.map((inv) => (
          <div key={inv.id} className="mobile-list-card" style={{ cursor: 'pointer' }} onClick={() => setEditing(inv)}>
        ...
        {editing && (
          <div className="mobile-form-card" style={{ border: '2px solid #2563eb', borderRadius: 16 }}>
            ...
            <div className="mobile-inline-actions" style={{ marginTop: 12 }}>
              ...
              <button type="button" className="mobile-secondary-button" onClick={() => setEditing(null)}>Sluiten</button>
            </div>
          </div>
        )}
