...
export function SuperadminControlCenter() {
  ...
  return (
    <MobilePageScaffold
      title="Superadmin"
      subtitle="Platformbeheer · Tenant 360"
      rightSlot={
        <button type="button" className="mobile-icon-button" onClick={load} disabled={loading || saving} aria-label="Vernieuwen">
          <RefreshCcw size={18} />
        </button>
      }
    >
      <div className="superadmin-v2-page superadmin-page">
      <style>{`.superadmin-v2-page{display:flex;flex-direction:column;gap:20px;color:var(--polish-text,#0f172a)}.sa-actions,.sa-tabs{display:flex;gap:10px;flex-wrap:wrap}.sa-button{border:1px solid rgba(49,95,170,.22);border-radius:12px;padding:10px 16px;font-weight:800;cursor:pointer;background:linear-gradient(135deg,#315faa 0%,#3f78d6 100%);color:#fff;box-shadow:0 10px 24px rgba(49,95,170,.2);display:inline-flex;align-items:center;gap:8px}.sa-button.secondary{background:#fff;color:#17356e;border:1px solid #d9e6f7;box-shadow:none}.sa-button.danger{background:#b42318;border-color:transparent;box-shadow:none}.sa-button:disabled{opacity:.55;cursor:not-allowed}.sa-card{background:linear-gradient(180deg,#fff 0%,#f9fbff 100%);border:1px solid #d9e6f7;border-radius:22px;padding:18px;box-shadow:0 14px 38px rgba(15,23,42,.08)}.sa-card h3{margin:0 0 14px;display:flex;align-items:center;gap:8px;font-size:18px;letter-spacing:-.02em;color:var(--polish-text,#0f172a)}.sa-kpi span,.sa-field span{font-size:13px;color:#66728a}.sa-kpi strong{display:block;margin-top:8px;font-size:28px}.sa-grid{display:grid;grid-template-columns:minmax(320px,420px) 1fr;gap:16px}.sa-toolbar{display:flex;gap:10px;margin-bottom:12px}.sa-search{display:flex;align-items:center;gap:8px;border:1px solid #d9e6f7;border-radius:12px;padding:0 10px;flex:1;background:#fff}.sa-input,.sa-select{border:0;outline:0;background:transparent;padding:10px 4px;width:100%;font:inherit}.sa-select{border:1px solid #d9e6f7;border-radius:12px;background:#fff;padding:10px}.sa-list{display:flex;flex-direction:column;gap:8px;max-height:760px;overflow:auto}.sa-row{border:1px solid #d9e6f7;border-radius:14px;background:#fff;padding:12px;text-align:left;cursor:pointer}.sa-row:hover{border-color:#9db7ff}.sa-row.is-active{border-color:#2563eb;box-shadow:0 0 0 3px rgba(37,99,235,.12)}.sa-row-top,.sa-table-row{display:flex;justify-content:space-between;gap:10px}.sa-row strong{display:block}.sa-row small{color:#66728a}.sa-pill{border-radius:999px;padding:4px 9px;font-size:12px;font-weight:800;background:#eef2f7;color:#45546d}.sa-pill.ok{background:#eaf8f0;color:#126b39}.sa-pill.warn{background:#fff7e6;color:#925300}.sa-pill.danger{background:#ffecec;color:#a51d16}.sa-detail{position:sticky;top:16px}.sa-detail-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.sa-field{background:#f8fafc;border:1px solid #d9e6f7;border-radius:14px;padding:12px}.sa-field strong{display:block;margin-top:4px}.sa-form{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;align-items:end}.sa-profile-form{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.sa-form label,.sa-profile-form label{display:flex;flex-direction:column;gap:6px;font-size:12px;font-weight:800;color:#66728a}.sa-form input,.sa-profile-form input,.sa-profile-form textarea{border:1px solid #dbe4f0;border-radius:12px;padding:10px;font:inherit}.sa-profile-form textarea{min-height:82px;resize:vertical}.sa-tabs button{border:1px solid #d9e6f7;background:#fff;border-radius:999px;padding:9px 13px;font-weight:800;cursor:pointer;color:#17356e}.sa-tabs button.active{background:linear-gradient(135deg,#315faa 0%,#3f78d6 100%);color:#fff;border-color:rgba(49,95,170,.35);box-shadow:0 8px 20px rgba(49,95,170,.18)}.sa-table{display:flex;flex-direction:column;gap:8px}.sa-table-row{align-items:center;border:1px solid #d9e6f7;border-radius:14px;padding:12px}.sa-table-row small{color:#66728a}.sa-audit{display:flex;flex-direction:column;gap:8px;max-height:340px;overflow:auto}.sa-alert{border-radius:14px;padding:12px 14px;font-weight:700;display:flex;gap:8px;align-items:center}.sa-alert.error{background:#fff1f0;color:#a51d16}.sa-alert.ok{background:#edfdf4;color:#126b39}.sa-empty{padding:22px;color:#66728a;text-align:center}@media(max-width:900px){.sa-grid{grid-template-columns:1fr}.sa-detail{position:static}.sa-detail-grid,.sa-form,.sa-profile-form{grid-template-columns:1fr}.sa-card{padding:14px}.sa-table-row{flex-direction:column;align-items:flex-start}}`}</style>

      {lastUpdated ? <div className="mobile-list-card-meta">Laatst bijgewerkt: {lastUpdated}</div> : null}
      ...
      <div className="mobile-kpi-grid">
        <div className="mobile-kpi-card mobile-kpi-card-primary">
          <div className="mobile-kpi-top"><span>Geschatte MRR</span></div>
          <strong>{money(revenue?.estimated_mrr)}</strong>
        </div>
        <div className="mobile-kpi-card mobile-kpi-card-success">
          <div className="mobile-kpi-top"><span>Geschatte ARR</span></div>
          <strong>{money(revenue?.estimated_arr)}</strong>
        </div>
        <div className="mobile-kpi-card mobile-kpi-card-warning">
          <div className="mobile-kpi-top"><span>Actieve tenants</span></div>
          <strong>{revenue?.active_tenants ?? '—'}</strong>
        </div>
      ...
      <div className="sa-actions" style={{ marginBottom: 4 }}>
        ...
        <button className="sa-button secondary" onClick={() => navigate('/superadmin/invoices')}><FileText size={16} /> Facturatie</button>
      </div>
