...
export function SuperadminPage() {
  ...
  return (
    <div className="page-stack">
      <ModuleHero
        title="Superadmin"
        description="Gebruik dezelfde header en stuur tenantbeheer, platformstatus en billing-acties aan vanuit duidelijke tegels."
        kicker="Platformcontrole"
        actions={
          <>
            <Button variant="secondary" onClick={() => tenants.refetch()}>Tenantlijst verversen</Button>
            <Button variant="secondary" onClick={() => health.refetch()}>Platformstatus verversen</Button>
          </>
        }
        tiles={[
          { label: 'Tenants', value: stats.totalTenants, meta: 'Volledige tenantlijst', icon: Building2, tone: 'primary' },
          { label: 'Actief', value: stats.activeTenants, meta: 'Actieve abonnementen', icon: BadgeCheck, tone: 'success' },
          { label: 'Gebruikers', value: stats.totalUsers, meta: 'Totaal platformgebruikers', icon: Users, tone: 'warning' },
          { label: 'Billing', value: selectedTenant ? (selectedTenant.name || selectedTenant.id) : 'Kies tenant', meta: 'Open detaildrawer op billing-tab', icon: CreditCard, onClick: () => { if (selectedTenant) setDetailTab('billing'); }, tone: 'neutral' },
        ]}
      />
      ...
      <div className="card-grid cols-2">
        <button type="button" className="module-hero-tile module-hero-tile-primary" onClick={() => tenants.refetch()} style={{ textAlign: 'left' }}>
          <div className="module-hero-tile-top"><Building2 size={18} /><span>Tenantbeheer</span></div>
          <strong>{stats.totalTenants}</strong>
          <small>Ververs tenantlijst en beheer tenants vanuit dezelfde tegelstructuur.</small>
        </button>
        <button type="button" className="module-hero-tile module-hero-tile-neutral" onClick={() => health.refetch()} style={{ textAlign: 'left' }}>
          <div className="module-hero-tile-top"><Activity size={18} /><span>Platformstatus</span></div>
          <strong>{health.isError ? 'Controle nodig' : 'Online'}</strong>
          <small>Gebruik dezelfde header en tegeltaal als de rest van het programma.</small>
        </button>
      </div>
