...
export function CeDossierPage() {
  ...
  return (
    <div className="page-stack">
      <PageHeader
        title="CE Dossier"
        description="Vereenvoudigd CE-overzicht op basis van de huidige live API (/api/v1/ce_export/{projectId})."
      />
      ...
      <Card>
        <div className="toolbar-cluster">
          <Button
            type="button"
            variant="secondary"
            onClick={() => ceQuery.refetch()}
            disabled={!projectId || ceQuery.isFetching}
          >
      ...
      {ceQuery.isError ? (
        <InlineMessage tone="danger">Het live endpoint /api/v1/ce_export/&#123;projectId&#125; geeft momenteel een fout of bestaat niet op de live API.</InlineMessage>
      ) : null}
        ...
        <>
          <div className="card-grid cols-4">
            <Card>
              <div className="metric-card">
                <span>Dossier gereed</span>
                <strong>{data.ready_for_export ? 'Ja' : 'Nee'}</strong>
              </div>
            </Card>
            <Card>
              <div className="metric-card">
                <span>Assemblies</span>
                <strong>{Number(counts.assemblies || assemblies.length || 0)}</strong>
              </div>
            </Card>
            <Card>
              <div className="metric-card">
                <span>Lassen</span>
                <strong>{Number(counts.welds || welds.length || 0)}</strong>
              </div>
            </Card>
            <Card>
              <div className="metric-card">
                <span>Inspecties</span>
                <strong>{Number(counts.inspections || inspections.length || 0)}</strong>
              </div>
            </Card>
          </div>
          ...
          <div className="content-grid-2">
            <Card>
              ...
              {data.project ? (
                <pre className="code-block">{JSON.stringify(data.project, null, 2)}</pre>
              ) : (
                <EmptyState
                  title="Geen projectinformatie"
                  description="De live API retourneerde geen project-object."
                />
              )}
            </Card>
            <Card>
              ...
              {photos.length ? (
                <pre className="code-block">{JSON.stringify(photos, null, 2)}</pre>
              ) : (
                <EmptyState
                  title="Geen foto’s"
                  description="Geen foto’s aanwezig in de live CE-exportresponse."
                />
              )}
            </Card>
          </div>
          ...
          <div className="content-grid-2">
            <Card>
              ...
              {assemblies.length ? (
                <pre className="code-block">{JSON.stringify(assemblies, null, 2)}</pre>
              ) : (
                <EmptyState
                  title="Geen assemblies"
                  description="Geen assemblies aanwezig in de live CE-exportresponse."
                />
              )}
            </Card>
            <Card>
              ...
              {welds.length ? (
                <pre className="code-block">{JSON.stringify(welds, null, 2)}</pre>
              ) : (
                <EmptyState
                  title="Geen lassen"
                  description="Geen lassen aanwezig in de live CE-exportresponse."
                />
              )}
            </Card>
          </div>
          ...
          <Card>
            ...
            {inspections.length ? (
              <pre className="code-block">{JSON.stringify(inspections, null, 2)}</pre>
            ) : (
              <EmptyState
                title="Geen inspecties"
                description="Geen inspecties aanwezig in de live CE-exportresponse."
              />
            )}
          </Card>
        </>
