...
export function ProjectenPage() {
  ...
  const projectStatusSummary = useMemo(() => {
    const summary = { total: rows.length, conform: 0, nonConform: 0, inProgress: 0, warning: 0 };
    rows.forEach((project) => {
      const status = String(project.status || '').toLowerCase();
      if (status.includes('conform') && !status.includes('non')) summary.conform += 1;
      else if (status.includes('non') || status.includes('defect') || status.includes('afkeur')) summary.nonConform += 1;
      else if (status.includes('progress') || status.includes('controle') || status.includes('review')) summary.inProgress += 1;
  ...
  return (
    <div className="page-stack">
      ...
      <div className="project-tab-kpi-grid">
        <ProjectKpiActionCard label="Projecten totaal" value={projectStatusSummary.total} meta="Volledige projectlijst" onClick={() => setSearch('')} />
        ...
        <ProjectKpiActionCard label="Niet conform" value={projectStatusSummary.nonConform} meta="Projecten met directe opvolging" onClick={() => setFilters((current) => ({ ...current, status: 'non_conform' }))} />
        <ProjectKpiActionCard label="In controle / aandacht" value={projectStatusSummary.inProgress + projectStatusSummary.warning} meta="Open acties of statuscontrole nodig" onClick={() => setFilters((current) => ({ ...current, status: 'in_progress' }))} />
      </div>
